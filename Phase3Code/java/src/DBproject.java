...
public class DBproject{
	...
	public static void AddDoctor(DBproject esql) {//1
		...
		try {
			query = "INSERT INTO Doctor (doctor_ID, name, specialty, did) VALUES (" + doctor_ID + ", \'" + name + "\', \'" + specialty + "\', " + did + ");";
	...
	public static void AddAppointment(DBproject esql) {//3
		...
		while (true) {
			...
			try {
				tempMonth = Integer.parseInt(in.readLine());
		...
		while (true) {
			...
			try {
				tempDay = Integer.parseInt(in.readLine());
		...
		while (true) {
			System.out.print("Input Year(1-9999):");
			try {
				tempYear = Integer.parseInt(in.readLine());
		...
		adate = tempMonth + "/" + tempDay + "/" + tempYear;
		
		int tempStart;
		int tempEnd;
		while (true) {
			System.out.print("Input Time Slot Start Hour(0 - 23):");
			try {
				tempStart = Integer.parseInt(in.readLine());
		...
		while (true) {
			System.out.print("Input Time Slot End Hour(0 - 23):");
			try {
				tempEnd = Integer.parseInt(in.readLine());
		...
		time_slot = tempStart + ":00-" + tempEnd + ":00";
		
		while (true) {
			System.out.print("Input Status(PA, AC, AV, WL): ");
			try {
				status = in.readLine();
		...
		try {
			query = "INSERT INTO Appointment (appnt_ID, adate, time_slot, status) VALUES (" + appnt_ID + ", \'" + adate + "\', \'" + time_slot + "\', \'" + status + "\);";
			esql.executeUpdate(query);
		}
		catch (Exception e)
		{
	...
	public static void FindPatientsCountWithStatus(DBproject esql) {//8
		...
		do {
			System.out.println("Please input the appointment number: ");
			try {
		...
		do {
			System.out.println("Please input the Patient status: ");
			try {
				status = in.readLine();
				break;
			} catch (Exception e) {
		...
		try {
			String query = "SELECT COUNT(*)\nFROM Appointment\nWHERE fid = " + num + " AND status = \'" + status + "\';";
